import { createBrowserHistory, BrowserHistoryBuildOptions, LocationState } from 'history';
import { matchRoutes, RouteConfig, MatchedRoute } from 'react-router-config';
import { ICustomRouter, IRouteMatchedEntry, IRoutingContext } from './types';
 ...
 */
export default function createRouter(routes: RouteConfig[], options?: BrowserHistoryBuildOptions): ICustomRouter {
  // Initialize history
  ...
  const context: IRoutingContext = {
    ...
    preloadCode(pathname: string) {
      ...
      matches.forEach(({ route }) => {
        ...
        return component ? component.load() : null
      });
    },
 ...
 */
function matchRoute(routes: RouteConfig[], location: LocationState): Array<MatchedRoute<{}>> {
 ...
 */
function prepareMatches(matches: Array<MatchedRoute<{}>>): Array<IRouteMatchedEntry> {
